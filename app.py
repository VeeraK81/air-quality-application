...
def preprocess_data(df):
    ...
    df["day"] = df["date_ech"].dt.day
    df["hour"] = df["date_ech"].dt.hour
    df["day_of_week"] = df["date_ech"].dt.dayofweek
...
def preprocess_input(input_data):
    df_input = pd.DataFrame([input_data])
    df_input["date_ech"] = pd.to_datetime(
        df_input["year"].astype(str) + "-" + df_input["month"].astype(str) + "-" + df_input["day"].astype(str)
    )
    df_input["year"] = df_input["date_ech"].dt.year
    df_input["month"] = df_input["date_ech"].dt.month
    df_input["day"] = df_input["date_ech"].dt.day
...
def upload_raw_data_s3(bucket_name, file_path, new_data):
    ...
    try:
        ...
        logger.info(f"File {file_path} not found in bucket {bucket_name}. Creating a new file.")
        existing_df = pd.DataFrame()
    ...
    logger.info(f"File {file_path} updated successfully in bucket {bucket_name}.")
...
def predict(input_data: InputData):
    try:
        date_obj = datetime.fromisoformat(input_data.date_input)
        year = date_obj.year
        month = date_obj.month
        day = date_obj.day
        hour = date_obj.hour
        ...
        input_dict = {
            ...
            "x_wgs84": input_data.x_wgs84,
            "y_wgs84": input_data.y_wgs84
        }
        ...
        response = {
            "date": input_data.date_input,
            "coordinates": {
                "x_wgs84": input_data.x_wgs84,
                "y_wgs84": input_data.y_wgs84
            },
            "category": map_predictions_to_labels(predictions[0])
        }
        ...
        upload_raw_data_s3(bucket_name, "transfer/air_quality_data/Air_Quality_Occitanie_Update.csv", new_data)
